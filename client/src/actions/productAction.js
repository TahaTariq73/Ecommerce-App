...
export const getProducts =
    (keyword = "", currentPage = 1, price = [0, (20 * 10000)], category, rating = 0) => async (dispatch) => {
        try {
...
// Delete review of a product --> Admin Route
...
export const deleteReviews = (productId, reviewId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(
            `/api/v1/review?id=${productId}&reviewId=${reviewId}`
        )
...
export const getDashboardStats = () => async (dispatch) => {
    try {
        dispatch({ type: ALL_STATS_REQUEST });
        
        const { data } = await axios.get(`/api/v1/admin/stats`);

        dispatch({ type: ALL_STATS_SUCCESS, payload: data });
    } catch (error) {
        dispatch({ type: ALL_STATS_FAIL, payload: error.response.data.message });
    }
}
